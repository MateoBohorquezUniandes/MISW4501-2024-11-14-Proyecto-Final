...
def check_healty_services():
    ...
    if(checksum_enable):
        ...
        write_to_output(f'{datetime.now()};{data_md5};{checksum};{"ok"if(checksum==data_md5) else "fail"}',"a")
    else:
        write_to_output(f'{datetime.now()};200;{status_code};{"ok"if(is_healthy) else "fail"}',"a")

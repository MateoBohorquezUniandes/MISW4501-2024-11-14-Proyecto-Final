        ...
        if usuario != "test":
            ...
            if user.get("nombre_usuario") != None:
                if(bcrypt.checkpw(password.encode("utf-8"), user.get("password").encode("utf-8"))):
                    expireAt = datetime.datetime.utcnow() + datetime.timedelta(minutes=7)
                    createdAt = datetime.datetime.utcnow()
                    extras = {"expireAt": expireAt,
                              "createdAt": createdAt}
                    token_de_acceso = create_access_token(identity=user.get("id"), expires_delta=datetime.timedelta(minutes=7), additional_claims=extras)
                    return {
            ...
            token_de_acceso = create_access_token(identity="0dbf8a3c-1611-4ef7-a985-4a7d2c7b983c", expires_delta=datetime.timedelta(minutes=7))
            expireAt = datetime.datetime.utcnow() + datetime.timedelta(hours=7)
            createdAt = datetime.datetime.utcnow()
            hashed = bcrypt.hashpw(password.encode("utf-8"), bcrypt.gensalt())
            passer = hashed.decode("utf-8")
            data_return = dict(accessToken=token_de_acceso,creator="test",createdAt=createdAt,expireAt=expireAt,mess=passer)
            return data_return
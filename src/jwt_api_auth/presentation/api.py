...
from jwt_api_auth.application.commands.create_token_alt import \
    CreateTokenHandler
from jwt_api_auth.application.mappers import CreateTokenJsonMapper
from jwt_api_auth.application.queries.validate_token import \
...
def create_token():
    ...
    if request_create.get("user") and request_create.get("password"):
        ...
        token = CreateTokenHandler().create(
            request_create.get("user"), request_create.get("password")
        )
        return token, 200
